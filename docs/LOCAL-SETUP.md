...
`python scripts/setup_azure_runtime.py --install --tools --prices` installs pinned Azure CLI/SDK into private environments and fetches public model pricing. Sign in interactively using `sh scripts/azure_pilot_az.sh login --use-device-code`. Configuration and token cache remain private. Do not copy another user's state.
