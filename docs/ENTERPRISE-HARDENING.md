...
| Sensitive model data | Actual OpenRouter key stays in trusted services. Chat, title generation, native Coder and gateway routing share `config/models.env`. The operator has temporarily set ZDR off; denied data collection remains on. Enforce ZDR again before sensitive enterprise use. | Approved transmission still sends data. Confirm account-level logging settings, contract, model/provider eligibility, region and data classes with the organization. Fail when no eligible provider exists; do not silently relax privacy. |
