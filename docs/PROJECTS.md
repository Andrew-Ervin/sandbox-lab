...
The environments have separate Coder control planes, namespaces, home volumes, credentials, packages and processes. Opening VS Code does not copy files or allocate headless compute. To use project source in VS Code, choose **Copy to VS Code** on the project page; to bring developer changes back, choose **Copy to chat**.

## Explicit sync

Both environments must already exist. A review wakes sleeping environments and lists eligible source files, sizes, and whether they are new, matching or different from the destination's live files. Approval copies exactly the reviewed bytes into a new `.lab/imports/transfer_<id>` directory. It never overwrites live source. Ask your coding agent to inspect and deliberately merge the import, or merge it yourself. Packages must be restored from manifests/lockfiles inside the destination.
...
Apps, files and VS Code open in the right pane. Opening it collapses history; reopening history collapses the preview. **Show preview** restores the existing iframe without restarting it. Collapsed views do not renew the preview lease. App servers still run in the same execution container where their source lives and follow its idle policy.
