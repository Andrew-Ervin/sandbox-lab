...
The browser UI is not offline: `app/page.tsx` loads the ChatKit runtime from `https://cdn.platform.openai.com/deployments/chatkit/chatkit.js` on startup. The root layout preconnects to that same host to reduce startup latency; this exposes normal connection metadata to the CDN, so browser egress is distinct from the execution-pod network policy. The preconnect itself carries no conversation payload. Removing only the resource hints would not remove the runtime dependency; a deployment that prohibits this browser destination needs a supported alternative distribution or UI.
...
Input bundling opens artifact directories and files relative to pinned file descriptors with `O_NOFOLLOW`, rejects nonregular files, and bounds both on-disk size and bytes read. Catalog size metadata is not trusted for enforcement. Swapped symlinks, oversized files, and file growth during a read cannot bypass the input limits. These are POSIX controls; Windows runs the backend through WSL2 as documented.
