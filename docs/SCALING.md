...
| Developer | 1 / 2 GiB default; 2 / 4 and 4 / 8 optional | 100 | 1,000 | 10 min | 60 min |
...
Supported Sandbox tiers are 0.25/0.5/20, 0.5/1/20, 1/2/20, 2/4/40 and 4/8/80 CPU/GiB/disk-GiB. The workspace capacity selector supports the three developer-ready tiers: 1/2/20, 2/4/40 and 4/8/80. CPU, memory and disk belong to the Sandbox tier, while the disk image is a separate reusable build; choosing a different tier does not require a new image. Azure snapshots cannot change CPU/memory on restore. Resizing therefore archives the saved home, stops the old sandbox, creates a sandbox at the requested tier, restores the home and reapplies the user's portable editor profile. The bounded segmented transfer preserves installed packages and native tool history; it can take minutes for large homes and refuses oversized transfers before stopping the original. Devices, FIFOs and external symlinks are excluded. The workspace ID and project association remain stable. Save editor buffers first; processes and terminals restart. The prior stopped sandbox remains recoverable and may incur retained-storage cost until explicit cleanup. There is no live vertical autoscaling. Allocation history is not CPU utilization; advisory downsizing uses separately sampled guest metrics described below. The current sandbox and Session Pool paths have no configured GPU. Ordinary Container Apps/Jobs have separate GPU options with regional and quota constraints; adopting one is a separate deployment, not a checkbox on this sandbox group.
...
Workstation activity checks now sample guest cgroup CPU usage and peak memory. After at least ten samples spanning five minutes, a suggestion can recommend a smaller tier with 50% headroom. It never changes compute or recommends an increase. CPU is an interval average and can miss short spikes; the sample is advisory, not a capacity guarantee. Missing cgroup metrics produce no recommendation.
