...
- Compared all original thread/project IDs and non-widget message JSON against the pre-review backup: preserved. Two completed approval widgets were refreshed without tool execution. The architecture app's existing reference files were updated in place.
...
- The same 156 tests also passed from a freshly extracted audited source archive, using the existing Python dependency environment but no copied `.env`, credentials or private experiment files. This verifies source independence, not a clean dependency/container installation.
