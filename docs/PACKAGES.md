...
|---|---|---|---|
| Quick Python | Configured built-in Python pool: verified NumPy, SciPy, Polars, Plotly and scikit-learn. The [custom image inventory](SCIENTIFIC-PYTHON.md) applies only to the fallback image. | Temporary run directory; networking denied | Session retained for 55 idle minutes; fresh interpreter per call. Saved files restore from checkpoints; variables and package installs do not persist. |
...
Quick compute uses a fresh Python process per call inside its conversation-scoped built-in session when configured. The built-in service prewarms allocation and expires the session after 55 idle minutes. It does not keep two long-running notebook kernels or install missing packages. A request for a missing package requires the AI to explicitly call delegate_project; the UI has no execution-mode selector. Ordinary Python computations avoid loading the chart renderer. Plotly show()/write_html() retain an interactive HTML chart. Static PNG rendering depends on the selected runtime; a missing renderer must not be presented as a successful image export.

The host stores exported file versions under `.local/artifacts/<run-id>/`. SQLite’s conversation file catalog points to the latest version of each filename in that conversation. Older runs keep their original copies. Selected quick inputs are copied to `/workspace/files`; selected project inputs go to `/home/sandbox/project/chat-inputs`. These are copies, not mounts of the Mac or another workspace. Outputs must be saved in the environment’s artifacts folder to appear in chat and Conversation files. Regular quick working files are checkpointed locally per conversation and reseeded next run (40 files / 16 MB total; hidden files and dependency trees excluded). Variables disappear after the call; saved files survive session expiry. Project files remain in the Azure home even when they are not exported to chat.

The lab caps each exported file at 8 MB and each run or input transfer at 16 MB. Saved run versions currently have no automatic expiration; they consume host disk until explicitly cleaned up. Azure’s idle stop affects compute, not saved files. Lockfiles can recreate dependencies after deletion, but do not recover unsaved source or datasets.

The server owns active ChatKit response streams. Switching conversations, opening Apps, or closing the browser does not cancel work. Configured quick executions and headless project turns can run concurrently; up to eight chat jobs make progress, with additional admitted jobs queued. These defaults are configurable within the Azure Sandbox capacity budget. Same-conversation turns are serialized. Stop run cancels only that conversation. Results and job status are saved in SQLite; a server restart interrupts in-flight work and retains completed results. This is one local broker process, not a distributed crash-resuming job service.
...
Local build hosts may use ARM64; the current Azure image uses Linux AMD64. C/C++ binaries, native npm modules, Python wheels, NuGet runtime assets and Julia artifacts must match the target architecture. Rebuild and restore for the actual Azure architecture instead of copying local binary caches. This .NET environment supports Linux console and ASP.NET applications; Windows-only WinForms/WPF and full .NET Framework require Windows environments.

## Verification and references

`scripts/smoke_languages.sh` compiles C/C++, then installs and executes a small dependency for Python, JS, Go, Rust, C# and Julia inside the headless Azure pod, and runs a shell script. It is a smoke test, not a guarantee that every package or native workload will fit the default resources.
...
The scientific inventory and reproducible update procedure are in [SCIENTIFIC-PYTHON.md](SCIENTIFIC-PYTHON.md). Research, advice and pricing questions use direct answers/web search, not execution. A tool-free routing check runs before proposed code execution; uncertain decisions do not allocate a run. This is a usability guard, not a security boundary.

Python wheel metadata probes use HEAD on catalogued artifact paths. The Azure relay preserves verified content length without a response body and applies the same age/checksum gate as GET. Other methods and arbitrary destinations remain denied.
