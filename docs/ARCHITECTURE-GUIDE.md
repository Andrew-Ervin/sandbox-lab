...
The model gateway uses operator-controlled routing in `config/models.env`, validates short-lived audience/model-scoped capabilities and bounds request bodies. ZDR is temporarily disabled; denied data collection remains enabled. AWS/OpenAI are preferred and Azure is excluded for LLM requests. There is no request-count allowance or usage ledger. Exa remains an administrator-controlled disclosure path. The current same-user Pi token remains reusable by scripts; preventing raw LLM access requires separating the trusted managed harness and its tools from code pods. Pi settings/extensions are not a locked administration boundary.
...
Projects now link an optional developer workspace alongside their separate chat workspace. Existing local developer workstations receive project records once; newly started workstations are linked automatically. Change project moves only that association; it does not move conversations or merge files. The project screen can open VS Code directly and shows the two compute states separately. Creating a developer link does not allocate headless compute. A source-bearing developer project can initialize its separate headless home on chat entry.
...
**Source synchronizes automatically in both directions.** Open in VS Code and New chat in project catch up eligible files before opening the destination. With both environments running, a trusted worker compares manifests every 15 seconds, waiting for active project coding to finish. One-sided changes and tracked deletions propagate; divergent edits remain until a Keep chat / Keep VS Code choice. Hash preconditions guard individual replacements, and baseline metadata survives restart. Background sync neither wakes sleeping pods nor renews user activity. An empty developer-only project needs no headless home; source-bearing developer projects allocate a separate one on chat entry.

Files, byte and traversal limits are in `config/limits.env`. Dependencies, credentials, build output, symlinks and recursive imports are excluded. Fresh projects use their project roots; legacy projects keep selected import paths. This is eventual source sync, not a multi-file transaction or shared execution. Packages and processes stay separate. See `PROJECTS.md` for conflict behavior and production limits.

Project deletion retains and unlinks its developer workstation, while deleting the headless home and project chat data. Deleting a developer workstation retains the project and its separate chat sandbox. The local owner model and filename filtering are not enterprise identity or full secret/DLP scanning.
...
Projects create or resume a linked workstation through Open in VS Code, automatically copying source and opening its folder. The GUI and headless filesystems remain separate; eligible source changes are synchronized. Creating the GUI link alone allocates no headless workspace; starting a project chat with existing developer source can allocate its separate headless home. See PROJECTS.md for transfer bounds and lifecycle. The chat header retains project navigation and this editor action; secondary preview controls are grouped in an options menu.

Opening the preview collapses history; opening history collapses the preview. Briefly hidden IDEs remain mounted, while expired previews resolve again on reopen. Approval cards use native decisions with a readable excerpt and an isolated request viewer with formatted Preview and full JSON tabs. Plain-text and HTML emails can show embedded raster images (including CID attachments); external images, scripts, active links and custom styles are disabled. Requests are bounded to 1 MB. Existing saved app/artifact/completed approval widgets can be refreshed without regenerating prose or running tools.
...
## Search sources and local credential renewal

Main chat converts OpenRouter Exa URL annotations into native ChatKit citation sources and persists them with each answer. Search is operator-controlled, bounded in `config/limits.env`, and excluded from title generation. It needs no assigned coding workspace. Source content is untrusted; search remains an outbound disclosure channel. See `MODEL-ROUTING.md`.

The host launcher checks the restricted quick broker credential every 30 seconds and renews its 24-hour token an hour before expiry. It writes an atomic 0600 kubeconfig containing only the broker identity. The trusted Python Kubernetes client reloads it for requests. Quick execution pods have no Kubernetes credential. AKS should replace host renewal with projected service-account rotation in the trusted broker.

## Execution activity and app navigation

Native Coder execution cards update with bounded provider-supplied reasoning, tool activity and token usage. Details remain with the owning conversation and are excluded from lightweight status polling. Missing reasoning text is not fabricated. See `NATIVE_CODER_TRIAL.md` and `LIMITS.md` for limits and production audit requirements.

Internal app/artifact prose links use ChatKit deeplinks to the authenticated in-app preview handler. The current project's latest saved app also remains accessible from Open app in the chat header during subsequent edits. Opening a preview never sends another chat message. Source edits do not themselves publish a separate service.

### Independent coding-session controls and recovery

The code-session strip below chat reads Coder's actual turn state, independently of the HTTP chat stream and the in-process chat job. **Stop coding** interrupts the displayed Coder turn and stops headless workspace compute, retaining its home volume. Coder interrupt alone can leave shell children running, so compute shutdown is the reliable termination boundary. Existing app previews close and can be started again on the next open. The separate GUI workstation is unaffected. A separate **Stop response** is available when only the main response is running. Same-project chats can surface an active sibling turn. The stop endpoint verifies conversation ownership and the expected session before acting, and uses the existing session/CSRF boundary.

Startup seeds the observer from saved Coder session bindings. Active or unconfirmed sessions stay protected from idle eviction and source sync until their actual state becomes terminal. An unknown state is shown as reconnecting, never as idle. New coding requests check for an unfinished project session before dispatch. Read-only API reconnection never resends the coding prompt. Completed code remains completed if final response generation or artifact retrieval fails; file collection retries separately. Native execution still has its configurable 900-second deadline, which is reported explicitly rather than as a generic service timeout.

The observer is local-process coordination. AKS requires shared leases/session records and a durable job controller, so replica restarts and competing workers cannot orphan or double-dispatch turns. Do not scale this SQLite/process-local prototype horizontally unchanged.
