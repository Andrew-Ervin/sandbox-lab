...
**Open in VS Code automatically copies project source and opens that copied folder.** It creates or resumes the linked GUI workspace and wakes the existing headless home only to read source. Projects without headless compute open the developer home without allocating one. A source-manifest hash identifies the copy; reopening unchanged chat source preserves developer edits in that folder. Changed chat source creates a new import folder, never overwriting an older copy. This is an explicit one-click handoff, not continuous mirroring.

**Copy to chat remains a reviewed snapshot import.** It reads the developer folder opened by the handoff, shows new/matching/different files, and requires approval of the exact bytes within five minutes. Ownership, project state, links and active coding jobs are rechecked. Checksummed source is published under a new `.lab/imports/transfer_…` directory in the headless home; merging that import remains deliberate. Installed dependencies, credentials and processes are not transferred.
...
Projects create or resume a linked workstation through Open in VS Code, automatically copying source and opening its folder. The GUI and headless filesystems remain separate; return copies to chat require review. Creating/opening that link allocates no headless workspace. See PROJECTS.md for transfer bounds and lifecycle. The chat header retains project navigation and this editor action; secondary preview controls are grouped in an options menu.
