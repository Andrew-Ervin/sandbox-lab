...
Before enterprise rollout, enable and centrally enforce `provider.zdr=true` / `data_collection=deny` and model/provider allow policies. The local prototype currently has ZDR temporarily disabled in `config/models.env` by operator choice; do not treat that as the production policy. Validate ZDR eligibility and contract/region/data-class requirements. Set account-level controls too, and disable optional prompt logging. Exa is a separate disclosure path: preserve only where the project classification permits it. Do not treat ZDR as DLP. Add payload minimization/redaction, secret detection and a reviewed export workflow for regulated data.
...
Retain distinct developer and headless workspace IDs, namespaces, identities and home storage. A link is organizational, not permission to execute chat code in the developer workstation. The local prototype synchronizes eligible source in both directions on project entry and every 15 seconds while both environments run. One-sided changes propagate; same-file conflicts require a user choice. It keeps a last-common hash baseline and checks destination hashes before replacing files. This is eventual per-file sync, not an atomic shared filesystem. Installed packages, credentials and processes do not move with that copy.
...
For AKS, replace the single-host source-sync worker and SQLite baseline with durable queued sync jobs, per-project distributed leases and revisioned storage. Define source classification/secret scanning, cross-platform filename policy, deletion retention and conflict resolution. Prefer Git commits for complex collaborative applications; maintain separate compute identities and package environments. Monitor scan/transfer latency and skipped/busy/conflict counts before increasing sync frequency. Use projected service-account rotation for the broker rather than a host process minting Kubernetes tokens.

Route execution diagnostics separately from content: retain structured timing/status/usage metrics in Azure Monitor/OpenTelemetry, and keep provider reasoning/tool payloads in owner-scoped conversation storage with explicit retention/redaction. Do not put prompts or credentials in generic request logs. Use short-lived workload and user grants, centralized refresh/revocation, and durable reconciliation; the local capability-file worker is a prototype, not Entra lifecycle management.

### Recover coding independently from chat transport

Persist the native session ID, workspace binding, dispatch idempotency state and current turn in the durable job controller. Track Coder state even when a web request or application replica disappears. Protect active/unknown sessions from eviction and source sync. Expose a user-authorized, CSRF-protected stop control separately from chat cancellation, bound to the current turn/session. Native interrupt alone may leave subprocesses running: use per-turn process groups/cgroups in a production executor, or stop headless compute as the local prototype does. Preserve project storage and allow app previews to restart on demand. Avoid trusting browser-provided workspace IDs. Replace this prototype's local in-memory observer locks with shared leases and a bounded worker queue; retain audit records for interrupts and reconciliation. Retry reads and result transfers; never automatically replay an ambiguous prompt or side-effecting tool call.
