...
class Identity:
    ...
    def login(self):
        ...
        params = {'client_id':self.config['client_id'],'response_type':'code','response_mode':'query',
            'redirect_uri':self.config['redirect_uri'],'scope':'openid profile email','state':state,'nonce':nonce,
    ...
    async def validate_token(self, raw, nonce):
        ...
        claims = jwt.decode(raw,jwt.PyJWK.from_dict(key).key,algorithms=['RS256'],audience=self.config['client_id'],
            issuer=f'https://login.microsoftonline.com/{tenant}/v2.0',options={'require':['exp','iat','nbf','aud','iss','tid','oid','nonce']},leeway=30)
        if claims['tid']!=tenant or not secrets.compare_digest(claims['nonce'],nonce): raise ValueError('Identity does not match sign-in')
    ...
    async def callback(self, request):
        ...
        try:
            async with httpx.AsyncClient(timeout=20) as client:
                ...
                claims = await self.validate_token(response.json()['id_token'],flow['nonce'])
        except Exception:
