...
def local_services(control):
    ...
    return {'storage':storage,'package_gateway':package,'retained':retained,'oldest_stopped_seconds':max(past,default=0),
            ...
            'session_pool':{'configured':bool(control.config.get('builtin_session_endpoint')),'idle_seconds':3300,'max_sessions':control.config.get('builtin_session_limit',10),'note':'Quick Python uses the configured built-in pool; headless handles larger tasks. Cloud inventory is authoritative for deployed resources.' if control.config.get('builtin_session_endpoint') else 'Quick Python uses the custom sandbox fallback.'}}
