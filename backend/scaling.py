...
class WarmPolicy:
    def __init__(self,base=2,maximum=4,idle_seconds=300,clock=time.monotonic,minimum=0):
        self.base=base;self.maximum=maximum;self.idle_seconds=idle_seconds;self.clock=clock
        self.minimum=max(0,min(minimum,maximum))
    ...
    def reserve(self,busy=0):
        ...
        if not busy and now>=self.warm_until and (self.last_activity is None or now-self.last_activity>=self.idle_seconds): return self.minimum
        rate=len(self.arrivals)/max(5,min(30,now-self.arrivals[0]+1)) if self.arrivals else 0
        return min(self.maximum,max(self.minimum,self.base,math.ceil(rate*self.refill_seconds*1.5)))
